...
#GUI oppsett 
root= tk.Tk()
root.title("Vektor kalkulator R1 v1.0")
...
def get_vektor():
    try:
        x1 = float(v1x.get() or "0")
        y1 = float(v1y.get() or "0")
        x2 = float(v2x.get() or "0")
        y2 = float(v2y.get() or "0")
        return x1, y1, x2, y2
    except ValueError:
...
def kalk_addisjon():
    ...
    if not values:
        return
    x1, y1, x2, y2 = values
    resultat.set(f"Addisjon: ({x1 + x2:.3f}, {y1 + y2:.3f})")
...
def kalk_sub():
    ...
    if not values:
        return
    x1, y1, x2, y2 = values
    resultat.set(f"Subtraksjon: ({x1 - x2:.3f}, {y1 - y2:.3f})")


#regner ut lengden av D med hjelp av AB
def kalk_lengde():
    values = get_vektor()
    if not values:
        return
    x1, y1, x2, y2 = values
    lengde = størrelse(x2 - x1, y2 - y1)
...
def kalk_vinkel():
    ...
    cos_theta = max(-1, min(1, cos_theta))  # klamper for å unngå feil i mattematikken
...
def kopier_resultat():
    ...
    resultat.set(original + " (kopiert!)")
    root.after(3000, lambda: resultat.set(original))  #resetter feltet etter 3 sekunder

#merkelapper/labels

lag_label(main_frame, "R1 Vektorkalkulator v1.0", row=9, column=0, columnspan=2,font=("Segoe UI", 9), style="Dark.TLabel")
...
#resultat
ttk.Label(main_frame, textvariable=resultat,font=("Segoe UI", 11, "bold"), style="Dark.TLabel").grid(row=8, column=0, columnspan=2, pady=10)
